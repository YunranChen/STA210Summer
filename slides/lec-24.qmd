...
If you want to apply `anova` to the fitted model from workflow, you have to apply `fitted_obj \%>\% extract\_fit\_engine()` to obtain fitted object that can be put into `anova()`.
