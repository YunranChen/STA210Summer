...
## Data manipulation 5: Transformation on variables {.smaller}
...
```{r}
loan50=loan50%>%
  mutate(annual_income_log=log(annual_income))
p3=ggplot(data=loan50,aes(x=annual_income_th))+
  geom_histogram()
p4=ggplot(data=loan50,aes(x=annual_income_log))+
  geom_histogram()
p3+p4
```
...
```{r}
p1=ggplot(data=loan50,aes(x=annual_income_th,y=interest_rate))+
  geom_point()

p2=ggplot(data=loan50,aes(x=annual_income_log,y=interest_rate))+
  geom_point()
p1+p2
```
...
log_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(interest_rate ~ debt_to_income + verified_income  + annual_income_log,
      data = loan50)
tidy(log_fit) %>%
  kable(digits = 3)
```
